...
1. 想找资料：先看课程页，不确定课程名就去 `Browse / 资料检索`
2. 想投稿：去 `Submit` 页面，按步骤填写后跳转 GitHub Issue
3. 网页里没看到资料：去 `static/files/...` 检查原文件是否已经在仓库里
...
- 先在 `Browse / 资料检索` 里搜关键词
- 再根据结果里的“所在位置”回到对应课程页
- 在课程页继续看同类资料
...
- 不需要自己提前设计仓库里的文件路径
- 如果选的是 `GitHub Issue 附件`，只需要把文件拖到 Issue 里
- 实际的仓库归档路径由维护时统一整理
...
如果你只是想确认“文件到底在不在仓库里”，优先看这里，不必先纠结网页显示是否正常。
...
- `static/files/...` 决定“文件在不在仓库里”
- `src/data/materials.ts` 决定“网页会不会把它显示出来”
- `src/data/courses.ts` 决定“课程页挂在哪个方向下面”
